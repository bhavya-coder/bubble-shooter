				...
				if (collisionBalls !== null) {
					...
					for (let i = 0; i < collisionBalls.length; i++) {
						...
						if (action === "BALLDESTROYED") {
							...
							// The first removed ball will be the next launcher
							// start point
					...
					// Re-compute next collisions from collision time since
					// blocks may have changed
					for (let i = 0; i < balls.length; i++) {
						let ball = balls[i];
						if(collisionBalls.indexOf(ball) >= 0) {
							// Balls in collisionBalls have already been
							// recomputed
							ball.computeNextCollision(obstacles);
							continue;
						}
						
						// Re-Compute only for balls which should have touch
						// deleted blocks
